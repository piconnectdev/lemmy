...
instance_id uuid references instance on update cascade on delete cascade;
...
create table federation_allowlist (
  id uuid NOT NULL DEFAULT next_uuid() primary key,
  instance_id uuid references instance on update cascade on delete cascade not null unique,
  published timestamp not null default now(),
  updated timestamp null
);
...
create table local_site (
  id uuid NOT NULL DEFAULT next_uuid() primary key,
  site_id uuid references site on update cascade on delete cascade not null unique,
...
create table local_site_rate_limit (
  id uuid NOT NULL DEFAULT next_uuid() primary key,
  local_site_id uuid references local_site on update cascade on delete cascade not null unique,
