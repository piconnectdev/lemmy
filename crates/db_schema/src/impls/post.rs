...
impl Crud for Post {
  type Form = PostForm;
  type IdType = PostId;
  fn read(conn: &PgConnection, post_id: Self::IdType) -> Result<Self, Error> {
    use crate::schema::post::dsl::*;
    post.find(post_id).first::<Self>(conn)
  }

  fn delete(conn: &PgConnection, post_id: Self::IdType) -> Result<usize, Error> {
    use crate::schema::post::dsl::*;
  ...
  fn update(conn: &PgConnection, post_id: Self::IdType, new_post: &PostForm) -> Result<Self, Error> {
    use crate::schema::post::dsl::*;
...
impl Post {
  ...
  fn update_tx(conn: &PgConnection, post_id: Self::IdType, txlink: &str) -> Result<Self, Error> {
    use crate::schema::post::dsl::*;
