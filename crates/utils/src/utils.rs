...
pub fn eth_verify(account: String, nonce: String, signature: String) -> bool {
  let message = eth_message(nonce.clone());
  let signature = &signature[2..];
  let signature_bytes = hex::decode(signature).unwrap();
  let recovery_id = signature_bytes[64] as i32 - 27;

  //println!("signature_bytes: {} {:?} {:?} {} ", account, nonce, signature, recovery_id);
